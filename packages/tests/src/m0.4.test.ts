...
describe('M0.4 — inert MCP gateway', () => {
  ...
  it('keeps production gateway composition limited to approved fixed tools without direct host-control imports', () => {
    ...
    expect((source.match(/registerTool\s*\(/g) ?? [])).toHaveLength(APPROVED_PRODUCTION_TOOLS.length - GIT_WORKFLOW_TOOLS.length + 1);
    expect(source).toContain('for (const [name, title, description, inputSchema] of workflowTools)');
    expect(source).not.toMatch(/workspace\.(?:delete|rename|move)|git\.(?:pull|run)/i);
    expect(source).not.toMatch(/registerTool\s*\(\s*['"][^'"]*(?:execute|shell|network|recovery)[^'"]*['"]/i);
